...
//  FarmersGuild
//
//  Created by Brian Pagani on 8/23/22.
//
...
class JumpingState : PlayerState {
    ...
    let textures : Array<SKTexture> = (0..<2).map({ return "\($0)"}).map(SKTexture.init)
    ...
    override func didEnter(from previousState: GKState?) {
        ...
        playerNode.run(.applyForce(CGVector(dx: 0, dy: 30), duration: 0.1))
...
class IdleState : PlayerState {
    ...
    let textures = SKTexture(imageNamed: "jump0")
    lazy var action = { SKAction.animate(with: [textures], timePerFrame: 0.1)} ()
...
class CheerState: PlayerState{
    let textures = SKTexture(imageNamed: "cheer0")
